    ...
    for model_name, model in models.items():
        for column_name in column_names:
            ...
            for vehicle_id in te_utils.get_vehicle_ids():
                ...
                print(f"Training model {model_filename} for vehicle {vehicle_id}")
                # use 1-week data for training
                last_timestamp = vehicle_data.index[-1]
                start_timestamp = last_timestamp - datetime.timedelta(days=10)
                ...
                print(f"Model {model_filename} trained for vehicle {vehicle_id}")
                # if column_name ==
                # filtered_lowpass_vd = te_utils.butter_lowpass_filter(vehicle_data, 2, 0.05, 2, column_name)
                print(f"Max size for model {model_filename} is {max_size}")
                ...
                print(f"Forecasting done for model {model_filename} for vehicle {vehicle_id}")
                print(f"Predicted {pred_result.shape[0]} rows for model {model_filename} for vehicle {vehicle_id}")
            ...
            print(f"Saving predictions for model {model_name} and column {column_name}")
